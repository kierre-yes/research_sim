...
public class BaselinePSO implements ISchedulingAlgorithm {
    ...
    private void updateParticles() {
        //linear
        double w = calculateLinearInertiaWeight();
    ...
    private double calculateLinearInertiaWeight() {
    ...
    private void updateVelocity(Particle p, double w, double c1, double c2) {
        ...
        // FIXED velocity limit (baseline - no adaptive clamping)
        double maxVel = parameters.getDouble(AlgorithmParameters.MAX_VELOCITY_INITIAL);
        ...
        for (int i = 0; i < vel.length; i++) {
            ...
            // Fixed velocity clamping (no adaptation)
    ...
    private Map<Cloudlet, Vm> convertToScheduleMap(double[] position) {
        ...
        LoadBalancer balancer = new LoadBalancer(schedule, vms, cloudlets);
        balancer.balance();
