...
public class EnhancedACO implements ISchedulingAlgorithm {
    ...
    public Map<Cloudlet, Vm> schedule(List<Cloudlet> cloudlets, List<Vm> vms, AlgorithmParameters parameters) {
        ...
        for (currentIteration = 0; currentIteration < parameters
                .getInt(AlgorithmParameters.MAX_ITERATIONS); currentIteration++) {
            ...
            // I check for early stopping based on fitness stagnation or pheromone
            // convergence
            if (shouldStopEarly()) {
                if (shouldDebug()) {
    ...
    private void calculateHeuristicMatrix() {
        ...
        for (int i = 0; i < cloudlets.size(); i++) {
            ...
            for (int j = 0; j < vms.size(); j++) {
                ...
                // I factor in resource availability so that ants prefer VMs with more available
                // resources
    ...
    private void constructSolutions() {
        // I let each ant build its solution probabilistically based on pheromones and
        // heuristics
        for (Ant ant : ants) {
            ant.constructSolution();

            // I evaluate each ant's solution so that we can identify good paths to
            // reinforce
    ...
    private void updateBestSolution() {
        ...
        if (parameters.getBoolean(AlgorithmParameters.ENABLE_EARLY_STOPPING)) {
            ...
            if (improvement < threshold) {
                ...
                // I reset the counter when improvement occurs so that temporary plateaus don't
                // trigger early stop
                stagnationCounter = 0;
            }
        }
    ...
    private double calculateAdaptiveEvaporation() {
        // I adjust evaporation rate based on pheromone entropy.
        ...
        if (vms.isEmpty() || cloudlets.isEmpty()) {
            return rhoMin;
        }
        ...
        double hMax = logFactorial(cloudlets.size()) + Math.log(vms.size());

        if (hMax == 0) {
            return rhoMin;
        }

        double normalizedEntropy = (h - hMin) / (hMax - hMin);

        normalizedEntropy = Math.max(0.0, Math.min(1.0, normalizedEntropy));
        double adaptiveRate = rhoMin + (rhoMax - rhoMin) * normalizedEntropy;
        return adaptiveRate;
    }
    ...
    private double calculateEntropy() {
        ...
        int rows = pheromoneMatrix.length;
        int cols = pheromoneMatrix[0].length;

        for (int i = 0; i < rows; i++) {
            double sumPheromone = 0.0;
            for (int j = 0; j < cols; j++) {
                sumPheromone += pheromoneMatrix[i][j];
            }

            double rowEntropy = 0.0;
            if (sumPheromone > 0) {
                for (int j = 0; j < cols; j++) {
                    double p = pheromoneMatrix[i][j] / sumPheromone;
                    if (p > 0) {
    ...
    // these methods were planned to be removed as they duplicate functionality in
    // AlgorithmMetricUtils
    // I switch now to AlgorithmMetricUtils.makespan(), cost(), energy(),
    // loadBalance() instead
